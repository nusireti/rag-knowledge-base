...
- **BM25+向量混合检索** 🔀 — 纯语义检索 + 关键词检索互补，准确率大幅提升
- **Cross-Encoder Rerank** 🎯 — 对检索结果重排序，相关性更高的排前面
- **多模型切换** 🔄 — UI 界面直接选择 Ollama 已安装的模型，不用改配置文件
- **对话搜索** 🔍 — 按标题搜索历史对话，快速找到之前的记录
- **Token 用量统计** 📊 — 顶部显示当前会话消耗的 Token 数
- **混合检索开关** ⚡ — 侧边栏一键切换纯向量检索/混合检索

### 🐛 问题 & 修复

#### 问题 14：AI 回答文字看不清（与背景对比度低）
- **修复**：`.chat-assistant` 背景从 0.06 调亮至 0.10，文字改为暖白 `#E8E8FF`，添加文字阴影
- **效果**：阅读舒适度大幅提升

#### 问题 15：GitHub 推送被拒（secret 泄露）
- **原因**：`list_repos.py` 中包含了 Personal Access Token
- **修复**：移除文件，添加到 .gitignore，`git commit --amend` 重写提交历史
...
- **Agent 模式** 🤖：AI 自动判断使用什么工具来回答问题
- **联网搜索** 🌐：集成 DuckDuckGo，知识库没有的内容自动上网搜
- **计算器** 🧮：直接算数学题，如 `sqrt(144) + 3.14 * 2`
- **当前时间** ⏰：AI 知道现在几点
- **模式切换**：侧边栏一键切换 RAG 模式 / Agent 模式
- **回答标注**：Agent 模式标记每个回答的来源（知识库/联网/直接回答）

### 🐛 问题 & 修复

#### 问题 13：AI 只能回答知识库里有的内容，无法处理实时信息
- **原因**：RAG 模式只检索本地文档，无法获取最新信息
- **解决方案**：新增 `agent_tools.py` 模块，实现 ReAct 模式的 Agent 系统
- **工作流程**：
  1. LLM 判断是否需要使用工具
  2. 需要 → 选择工具并生成输入
  3. 执行工具 → 获得结果
...
- **多知识库切换**：支持创建、切换、删除多个独立知识库，每个 KB 有独立的文档和向量库
- **文档预览**：侧边栏可直接预览文档内容（TXT/MD/PDF/DOCX），不用打开外部程序
- **知识库管理**：新建 KB、删除 KB、快速切换，数据完全隔离
- **动态目录**：ingest/query 引擎支持动态指定向量库目录

### 🐛 问题 & 修复

#### 问题 12：只有一个知识库，想换一批文档就得删掉重来
- **解决方案**：新增 `kb_manager.py` 模块，实现多知识库独立管理
- **架构**：
  ```
  knowledge_bases/
  ├── default/           # 默认知识库
...
| 版本 | 计划功能 | 状态 |
|------|---------|------|
| v1.0 ~ v2.2 | 基础 RAG → 多知识库 → Agent | ✅ 已完成 |
| v3.0 | 用户认证 + 数据库 + Docker | ✅ 已完成 |
| **v3.5** | **BM25混合检索 + 多模型切换 + 对话搜索** | **✅ 已完成** |
| v4.0 | 多模型 API 接入（OpenAI / 文心 / 通义） | 📅 计划中 |
| v4.1 | API 后端（FastAPI）、RESTful 接口 | 📅 计划中 |
| v5.0 | 多模态（图片识别/语音）、移动端适配 | 🎯 远期目标 |
